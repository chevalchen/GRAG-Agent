...
class KimiRecipeAgent:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.moonshot.cn/v1"):
        ...
        if (LANGCHAIN_TRACING_V2 or LANGSMITH_TRACING):
            try:
    ...
    def extract_recipe_info(self, markdown_content: str, file_path: str = "") -> RecipeInfo:
        ...
        category_field = inferred_category or '菜谱分类（素菜/荤菜/水产/早餐/主食/汤类/甜品/饮料/调料，支持多个分类用逗号分隔，如"早餐,素菜"）'
...
def main():
    ...
    parser.add_argument('-k', '--api-key', required=False, help='Kimi API密钥')
    ...
    api_key = args.api_key or KIMI_API_KEY or MOONSHOT_API_KEY
    if not api_key:
        print("错误: 未找到Kimi API密钥")
        return
    
    print("初始化Kimi AI Agent...")
    ai_agent = KimiRecipeAgent(api_key, args.base_url)
