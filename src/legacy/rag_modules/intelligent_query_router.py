...
class IntelligentQueryRouter:
    ...
    def route_query(self, query: str, top_k: int = 5) -> Tuple[List[Document], QueryAnalysis]:
        ...
        try:
            documents = self.retrieve_with_analysis(query, analysis, top_k)
            documents = self.post_process_results(documents, analysis)
    ...
    def retrieve_with_analysis(self, query: str, analysis: QueryAnalysis, top_k: int) -> List[Document]:
        self.record_route(analysis.recommended_strategy)
    ...
    def post_process_results(self, documents: List[Document], analysis: QueryAnalysis) -> List[Document]:
        return self._post_process_results(documents, analysis)
    ...
    def record_route(self, strategy: SearchStrategy):
