...
def _make_stub_tools(
    ...
    hybrid_docs = hybrid_docs or [
        Document(page_content="H", metadata={"node_id": "n1", "chunk_id": "c1", "recipe_name": "H菜"})
    ]
    graph_docs = graph_docs or [
        Document(page_content="G", metadata={"node_id": "n2", "chunk_id": "c2", "recipe_name": "G菜"})
    ]

    llm = MagicMock(spec=LLMGenerationTool)

    def _invoke_text(prompt: str) -> str:
        if "recommended_strategy" in prompt:
            return (
                '{"intent": "test", "keywords": ["测试"], '
                f'"query_complexity": 0.1, "relationship_intensity": 0.1, '
                f'"recommended_strategy": "{supervisor_strategy}"}}'
            )
        return answer_text
...
class TestAnalysisField(unittest.TestCase):
    def test_analysis_is_query_analysis_dataclass(self):
        graph, *_ = _build("hybrid")
        state = graph.invoke(
        ...
        self.assertIn(analysis.recommended_strategy, {"hybrid", "graph_rag", "combined"})

    def test_empty_query_returns_default_strategy(self):
        graph, *_ = _build("hybrid")
        state = graph.invoke(
            {"query": "", "metrics": {}},
        ...
        analysis = state.get("analysis")
        self.assertIsNotNone(analysis)
        self.assertEqual(analysis.recommended_strategy, "hybrid")
